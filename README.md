...
登录后点击顶部“服务器金额”旁的金额设置按钮，可以切换总价、每月、每日，以及人民币或美元显示。节点金额可以单独选择是否跟随统计栏的显示货币，也可以选择是否跟随上方的计费周期；关闭时保留每台服务器原本的货币和计费周期。汇率默认每天从 [Frankfurter](https://frankfurter.dev/) 获取；如果需要固定汇率，可关闭“自动更新汇率”，按 `USD=7.2,CAD=5.0` 的格式填写手动汇率。节点的货币字段建议使用 ISO 代码（例如 `CAD`），仅填写 `$` 时无法区分美元和加元。管理员还可以在 PurCarte 主题管理中设置卡片价格标签登录限制，以及上述价格选项的默认值。
