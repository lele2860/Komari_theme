...
登录后点击顶部“服务器金额”旁的金额设置按钮，可以切换总价、每月、每日，以及“默认（原本货币）”、人民币或美元显示。开启“节点金额统一显示”后，节点金额会跟随上方选中的计费周期；关闭后保留每台服务器原本的计费周期。汇率默认每天从 [Frankfurter](https://frankfurter.dev/) 获取；如果需要固定汇率，可关闭“自动更新汇率”，按 `USD=7.2,CAD=5.0` 的格式填写手动汇率。节点的货币字段建议使用 ISO 代码（例如 `CAD`），仅填写 `$` 时无法区分美元和加元。
